  ...
  return (
    <div className={styles.card}>
      ...
      <div className={styles.options}>
        {[{ label: "YES", percent: yesPercent }, { label: "NO", percent: noPercent }].map((opt) => (
...
export default function Dashboard() {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const updatedUser = await fetch(`http://localhost:8080/api/users/${user.id}`).then((r) =>
        r.json()
      );
      login(updatedUser);
      ...
      <nav className={styles.navbar}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
          <div className={styles.balanceChip}>
            🪙 <strong>{Number(user?.balance || 0).toLocaleString()}</strong> coins
          </div>
          <button
      ...
      {modal && (
        <div className={styles.overlay} onClick={() => setModal(null)}>
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            {betError && (
              <p style={{ color: "red", fontSize: "13px", margin: 0 }}>{betError}</p>
            )}
